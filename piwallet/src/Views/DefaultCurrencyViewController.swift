...
class DefaultCurrencyViewController: UITableViewController, Subscriber {
  ...
  init(walletManager: WalletManager, store: Store) {
    ...
    self.store = store
    self.rates = store.state.rates.filter {
      $0.code != C.btcCurrencyCode
    }
  ...
  private let bitcoinSwitch = UISegmentedControl(items: ["subIVCs (\(S.Symbols.bits))", "IVC (\(S.Symbols.btc))"])
