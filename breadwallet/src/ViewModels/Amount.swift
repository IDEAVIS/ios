...
struct Amount {
  ...
  var bits: String {
    ...
    guard let string = btcFormat.string(from: number) else {
      return ""
    }
    return string
  }
  ...
  var localCurrency: String {
    guard let string = localFormat.string(from: Double(amount) / 100000000.0 * rate.rate as NSNumber) else {
      return ""
    }
    return string
  }
  ...
  func string(forLocal local: Locale) -> String {
    ...
    guard let string = format.string(from: Double(amount) / 100000000.0 * rate.rate as NSNumber) else {
      return ""
    }
    return string
  }
...
struct DisplayAmount {
  ...
  private var fiatDescription: String {
    guard let rate = selectedRate ?? state.currentRate else {
      return ""
    }
    guard let string = localFormat.string(from: Double(amount.rawValue) / 100000000.0 * rate.rate as NSNumber) else {
      return ""
    }
    return string
  }
